        ...
        <div class="max-w-md mx-auto border-4 shadow-xl bg-white/90 border-amber-300/50 sm:rounded-lg sm:p-6 lg:p-6">
                ...
                <form method="POST" action="{{ route('departments.update',$department->id) }}">
                    ...
                    <div>
                        ...
                        <x-jet-input id="name" class="block w-full mt-1" type="text" name="name" value="{{ $department->name }}" required autofocus autocomplete="name" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="den_id" value="{{ __('Assign Dean') }}" />
                        <select id="dean_id" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-amber-300 focus:ring focus:ring-amber-300/50" name="dean_id" required autofocus>
                            ...
                            <option value="{{ $user->id }}" {{ $department->dean_id == $user->id ? ' selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
