        ...
        <div class="max-w-md mx-auto border-4 shadow-xl bg-white/90 border-amber-300/50 sm:rounded-lg sm:p-6 lg:p-6">
                ...
                <form method="POST" action="{{ route('students.update',$student->id) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <x-jet-input id="student_number" class="block w-full mt-1" type="text" name="student_number" value="{{ $student->student_number }}" required autofocus autocomplete="name" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="name" value="{{ __('Name') }}" />
                        <x-jet-input id="name" class="block w-full mt-1" type="text" name="name" value="{{ $student->name }}" required autofocus autocomplete="name" />
                    </div>

                    <div class="mt-4">
                        <x-jet-label for="email" value="{{ __('Email') }}" />
                        <x-jet-input id="email" class="block w-full mt-1" type="email" name="email" value="{{ $student->email }}" required />
                    </div>
